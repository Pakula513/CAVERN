...
state : State = State.FOLLOWING_LEFT
...
def update():
    global state, cur_path, target_path, readyToReverse, index, numIntersectionsPassed, numIntersectionsToPass, time_since_last_ar
    """
    ...
    if ar_marker is not None:
        print('found some ar marker')
        if time_since_last_ar > 2:
            print('found new ar marker')
            if state != State.FOLLOWING_OTHER_CAR and state != State.FINISHED_HIGH_PRIORITY:
                if ar_marker.get_id() == END_OF_PATH_ID:
                    ...
                    if state == State.REVERSING_RIGHT:
                        cur_path.pop()
                        if cur_path[-1] == 1:
                            state = State.REVERSING_RIGHT
                        else:
